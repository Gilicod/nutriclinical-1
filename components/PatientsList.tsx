  ...
  const handleSavePatient = () => {
    ...
    const newPatient: Patient = {
        ...
        lifestyle: {
            ...
            diet: { meals: '', water: '', alcohol: formData.alcoholTobacco.includes('Alcohol'), tobacco: formData.alcoholTobacco.includes('Tabaco') },
    ...
    <div className="space-y-6">
                ...
                <tr key={patient.id} className="hover:bg-slate-800/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-blue-900/50 text-blue-200 flex items-center justify-center text-sm font-bold overflow-hidden">
                  ...
                  <td className="px-6 py-4 text-slate-400">{patient.email}</td>
                  <td className="px-6 py-4 text-slate-400">{patient.phone}</td>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
            <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-6xl shadow-2xl flex flex-col my-auto max-h-[95vh]">
                {/* Header */}
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <textarea 
                                            ...
                                            className="w-full bg-slate-800 border border-slate-600 rounded-lg p-2 text-white text-sm h-20 resize-none focus:border-blue-500 outline-none"
                                        />
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="lg:col-span-7 space-y-6">
                            ...
                            <div className="bg-slate-800/40 p-5 rounded-xl border border-slate-700">
                                ...
                                <div className="grid grid-cols-2 gap-y-2 gap-x-4 mb-4">
                                    ...
                                    <Checkbox label="Hipertensión Arterial" checked={formData.hypertension} onChange={c => handleCreateChange('hypertension', c)} />
                                    <Checkbox label="Enfermedades Renales" checked={formData.renal} onChange={c => handleCreateChange('renal', c)} />
                                </div>
                            ...
                            {/* Gineco-obstétricos (Only show if Female ideally, but keeping static for UI consistency with screenshot) */}
                            <div className="bg-slate-800/40 p-5 rounded-xl border border-slate-700">
                            ...
                            <div className="bg-slate-800/40 p-5 rounded-xl border border-slate-700">
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-4">
                                    ...
                                    <Input label="Alcohol/Tabaco" value={formData.alcoholTobacco} onChange={v => handleCreateChange('alcoholTobacco', v)} />
                                </div>
                ...
                <div className="p-4 border-t border-slate-800 flex justify-end gap-3 bg-slate-900 shrink-0 rounded-b-xl">
                    ...
                    <button onClick={handleSavePatient} className="bg-[#0085db] hover:bg-[#0070b8] text-white px-8 py-2 rounded-lg font-bold flex items-center gap-2 shadow-lg">
...
const Input = ({ label, value, onChange, type = "text" }: any) => (
    <div>
        ...
        <input 
            ...
            className="w-full bg-slate-800 border border-slate-600 rounded-lg p-2 text-white text-sm focus:border-blue-500 outline-none transition-colors"
        />
    </div>
);
...
const Checkbox = ({ label, checked, onChange }: any) => (
    <label className="flex items-center gap-2 cursor-pointer group">
        <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${checked ? 'bg-blue-600 border-blue-600' : 'bg-slate-800 border-slate-600 group-hover:border-blue-500'}`}>
