...
const CalendarIcon = ({ date }: { date: string }) => {
    const d = new Date(date + 'T12:00:00'); // Force midday to avoid timezone shifts on just dates
    return (
        <div className="flex items-center gap-2 font-semibold">
            <Calendar size={18} className="text-blue-500" />
            <span className="capitalize">{d.toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</span>
        </div>
    )
}